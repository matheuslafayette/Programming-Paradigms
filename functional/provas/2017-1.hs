...
bSort :: Ord t => [t] -> [t]
bSort xs = bubbleAux xs ((length xs) - 1)

bubbleAux :: Ord t => [t] -> Int -> [t]
bubbleAux xs 0 = xs
bubbleAux xs n = bubbleAux (bubbleSort xs) (n-1)

bubbleSort :: Ord t => [t] -> [t]
bubbleSort [] = []
bubbleSort [x] = [x]
bubbleSort (x:y:zs) | x <= y = x:(bubbleSort (y:zs))
                    | otherwise = y:(bubbleSort (x:zs))
...
isSortedTree (Node v subtr1 subtr2) = v >= (foldr1 max ls) && v <= (foldr1 min rs) && isSortedTree subtr1 && isSortedTree subtr2
    where
        ls = v:(collapse subtr1)
        rs = v:(collapse subtr2)

collapse :: Tree t -> [t]
collapse (Leaf value) = [value]
